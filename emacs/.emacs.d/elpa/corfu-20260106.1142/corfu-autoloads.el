...
With prefix ARG, enable Global Corfu mode if ARG is
...
Corfu mode is enabled in all buffers where `corfu--on' would
do it.

See `corfu-mode' for more information on Corfu mode.

`global-corfu-modes' is used to control which modes this
